...
class MyStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.logger = Logger.instance()
        self.previous_day_close_price = None
    ...
    def next(self):
        ...
        cash_for_purchase = 10000

        final_close_price = self.dataclose[0]
        if self.previous_day_close_price and self.previous_day_close_price > self.dataclose[0]:
            buy_qty = math.floor(cash_for_purchase / self.data)
            self.buy(size=buy_qty)
            self.log(f'BUY, {self.dataclose[0]:,.2f}, {buy_qty}')
        self.previous_day_close_price = self.dataclose[0]
...
def run_main():
    global ticker, last_n_years, initial_cash, max_lots
